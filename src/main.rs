    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rockywebde=info,tower_http=warn".into()),
        )
        .init();
...
async fn serve() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let addr = std::env::var("RWDE_LISTEN").unwrap_or_else(|_| "0.0.0.0:7788".into());
    let listener = tokio::net::TcpListener::bind(&addr).await?;
    tracing::info!("rockywebde listening on http://{addr}");
